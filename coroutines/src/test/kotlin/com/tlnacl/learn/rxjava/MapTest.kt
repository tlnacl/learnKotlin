...
class MapTest {
    ...
    class TestTransformer : ObservableTransformer<String, String> {
        private fun preString(s: String): Single<String> {
            return Single.just("pre-$s")
                    .doOnSuccess { pre: String -> println("add pre:$pre") }
        }

        private fun postString(s: String): Single<String> {
            return Single.just("$s-post")
                    .doOnSuccess { post: String -> println("add post:$post") }
        }

        override fun apply(upstream: Observable<String>): ObservableSource<String> {
            return upstream
                    .concatMap { s: String -> preString(s).flatMap { s: String -> postString(s) }.toObservable() }
        }
    }
}